...
import { Pencil, Info, ChevronDown, Trash2 } from "lucide-react";
...
export default function EquipmentExperienceForm() {
  ...
  const removeEquipment = (id: string) => {
    // FIX: Removed the records.length > 1 check to allow deleting any entry
        ...
        {records.map((record) => {
          ...
          if (record.isSaved) {
            ...
            return (
              <div
                key={record.id}
                className="bg-[#F8FAFC] border border-[#E2E8F0] rounded-xl p-4 md:p-5 flex flex-col md:flex-row items-start md:items-end gap-4 transition-all relative group"
              >
                <div className="grid grid-cols-1 sm:grid-cols-7 gap-4 w-full pr-0 md:pr-10">
                ...
                <div className="flex items-center gap-2 w-full md:w-auto shrink-0 justify-end">
                  <Button
                    ...
                    className="h-11 px-4 gap-2 bg-[#F1F5F9] text-[#1E293B] hover:bg-slate-200 border border-slate-200/60 rounded-lg text-sm font-medium font-inter shadow-sm flex-1 md:flex-initial"
                  >
                    <Pencil className="h-4 w-4 text-[#334155]" strokeWidth={2} />
                    Edit
                  </Button>
                  
                  {/* FIX: Removed records.length > 1 check so trash button is always visible */}
                  <Button
                    type="button"
                    variant="ghost"
                    onClick={() => removeEquipment(record.id)}
                    className="h-11 px-3 text-slate-400 hover:text-red-500 hover:bg-red-50 border border-slate-200/60 hover:border-red-200 rounded-lg transition-colors cursor-pointer shrink-0"
                    title="Delete Entry"
                  >
          ...
          return (
            <div
              key={record.id}
              className="border border-[#E2E8F0] rounded-xl bg-white p-4 sm:p-6 space-y-5 relative"
            >
              {/* FIX: Removed records.length > 1 check so absolute trash button is always visible */}
              <button
                type="button"
                onClick={() => removeEquipment(record.id)}
                className="absolute right-4 top-4 text-slate-400 hover:text-red-500 hover:bg-red-50 p-2 rounded-lg border border-transparent hover:border-red-100 transition-all flex items-center justify-center cursor-pointer z-10"
                title="Remove Equipment Block"
              >
                <Trash2 className="h-5 w-5" />
              </button>

              {/* Row 1: Dropdowns */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-6 sm:pt-4">
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-end gap-3 pt-3 border-t border-[#F1F5F9]">
                <Button
                  type="button"
                  variant="ghost"
                ...
                <Button
                  ...
                  className={`font-semibold font-inter text-sm h-11 px-6 rounded-lg transition-all shadow-sm ${
                    isSaveDisabled
